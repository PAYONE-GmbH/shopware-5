...
class AddressCheck implements SubscriberInterface
{
  ...
  public function sAdmin__sManageRisks__after(Enlight_Hook_HookArgs $arguments)
  {
        ...
        if (strpos($rule, 'sRiskMOPT_PAYONE__') !== 0) {
            $arguments->setReturn(
                    $arguments->getSubject()->executeParent(
                            $arguments->getMethod(), $arguments->getArgs()
            ));
        } else {
    ...
    protected function performConsumerScoreCheck($config, $addressData, $paymentID = 0)
    {
        $moptPayoneMain = $this->container->get('MoptPayoneMain');
    ...
    protected function handleBillingAddressCheckResult($response, $config, $userId, $caller, $billingAddressData)
  {
    ...
    if ($response->getStatus() == \Payone_Api_Enum_ResponseType::VALID)
    {
      ...
      if ($secStatus == \Payone_Api_Enum_AddressCheckSecstatus::ADDRESS_CORRECT)
      {
        ...
        $moptPayoneMain->getHelper()->saveAddressCheckResult('billing', $userId, $response, $mappedPersonStatus);
      }
      else
      {
  ...
  protected function handleShippingAddressCheckResult($response, $config, $userId, $subject, $shippingAddressData)
  {
    ...
    if ($response->getStatus() == \Payone_Api_Enum_ResponseType::VALID)
    {
      ...
      {
        //valid address returned -> save result to db
        $moptPayoneMain->getHelper()->saveAddressCheckResult('shipping', $userId, $response, $mappedPersonStatus);
      }
      else
      {
        //secstatus must be 20 - corrected address returned
  ...
  public function onConfirmAction(Enlight_Hook_HookArgs $arguments)
  {
    ...
    $billingAddressChecktype = $moptPayoneMain->getHelper()->isBillingAddressToBeCheckedWithBasketValue($config, 
            $basketValue, $billingAddressData['country']);
    ...
    if ($billingAddressChecktype && !$moptPayoneMain->getHelper()->isBillingAddressCheckValid($config['adresscheckLifetime'], $userBillingAddressCheckData['moptPayoneAddresscheckResult'], $userBillingAddressCheckData['moptPayoneAddresscheckDate']))
    {
      ...
      $response = $this->performAddressCheck($config, $params, $this->container->get('MoptPayoneBuilder'), $moptPayoneMain, $billingAddressChecktype);
    ...
    $shippingAddressChecktype = $moptPayoneMain->getHelper()->isShippingAddressToBeCheckedWithBasketValue($config, 
            $basketValue, $shippingAddressData['country']);
    ...
    if ($shippingAddressChecktype && !$moptPayoneMain->getHelper()->isShippingAddressCheckValid($config['adresscheckLifetime'], $shippingAttributes['moptPayoneAddresscheckResult'], $shippingAttributes['moptPayoneAddresscheckDate']))
    {
      ...
      $response = $this->performAddressCheck($config, $params, $this->container->get('MoptPayoneBuilder'), $moptPayoneMain, $shippingAddressChecktype);
    ...
    protected function getCustomerCheckIsNeeded($config, $userId, $basketAmount, $paymentName)
    {
        ...
        $needsRecompution = !$moptPayoneMain->getHelper()
                ->isConsumerScoreCheckValid($config['consumerscoreLifetime'], 
  ...
  public function onValidateStep2(Enlight_Hook_HookArgs $arguments)
  {
    ...
    if ($billingAddressChecktype)
    {
      ...
      else
      {
        ...
        $response = $this->performAddressCheck($config, $params, $this->container->get('MoptPayoneBuilder'), 
  ...
  public function onUpdateBilling(Enlight_Hook_HookArgs $arguments)
  {
    ...
    else
    {
      ...
      $mappedPersonStatus = $moptPayoneMain->getHelper()->getUserScoringColorFromValue($mappedPersonStatus);
      $moptPayoneMain->getHelper()->saveAddressCheckResult('billing', $userId, $result, $mappedPersonStatus);
    }
    ...
    if ($config['adresscheckActive'])
    {
      $shippingAddressChecktype = $moptPayoneMain->getHelper()
              ->getAddressChecktypeFromId($config['adresscheckShippingAdress'], 
                      $config['adresscheckShippingCountries'], $shippingAddressCountry);
  ...
  public function onUpdateShipping(Enlight_Hook_HookArgs $arguments)
  {
    ...
    else
    {
      ...
      $mappedPersonStatus = $moptPayoneMain->getHelper()->getUserScoringValue($result->getPersonstatus(), $config);
      $mappedPersonStatus = $moptPayoneMain->getHelper()->getUserScoringColorFromValue($mappedPersonStatus);
      $moptPayoneMain->getHelper()->saveAddressCheckResult('shipping', $userId, $result, $mappedPersonStatus);
    }
