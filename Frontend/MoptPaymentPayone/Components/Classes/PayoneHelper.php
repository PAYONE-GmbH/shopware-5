...
class Mopt_PayoneHelper
{
   ...
   * @return boolean
   */
  public function isShippingAddressToBeCheckedWithBasketValue($config, $basketValue, $selectedCountry)
  {
    if (!$config['adresscheckActive'])
    {
      return false;
    ...
    $shippingAddressChecktype = $this->getAddressChecktypeFromId($config['adresscheckShippingAdress'], 
            $config['adresscheckShippingCountries'], $selectedCountry);
   ...
   * @return boolean 
   */
  public function isShippingAddressCheckValid($adresscheckLifetime, $moptPayoneAddresscheckResult, 
  ...
  public function getShippingAddressAttributesFromUserId($userId)
  {
    ...
    if ($result)
    {
      ...
      $shippingAttributes['moptPayoneAddresscheckDate']   = DateTime::createFromFormat('Y-m-d', 
  ...
  public function getConsumerScoreDataFromUserId($userId)
  {
    ...
    if ($result)
    {
      ...
      $userCconsumerScoreData['moptPayoneConsumerscoreDate']   = DateTime::createFromFormat('Y-m-d', 
  ...
  public function getBillingAddresscheckDataFromUserId($userId)
  {
    ...
    if ($result)
    {
      ...
      $userBillingAddressCheckData['moptPayoneAddresscheckDate']   = DateTime::createFromFormat('Y-m-d', 
              $result[0]['mopt_payone_addresscheck_date']);
    }
