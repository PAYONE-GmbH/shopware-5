...
<div class="form-group has-feedback has-error menu-level-experte">
    ...
    <div class="col-md-6">
        <select class="form-control " pattern='{$pattern}' id="{$id}" name="{$id}" aria-describedby="{$id}-status" >
            <option value="0">{s name="fieldvalue/yes"}Ja{/s}</option>
            <option value="1">{s name="fieldvalue/no"}Nein{/s}</option>
            <option value="2">{s name="fieldvalue/userchoice"}Benutzerentscheidung{/s}</option>
        </select>
