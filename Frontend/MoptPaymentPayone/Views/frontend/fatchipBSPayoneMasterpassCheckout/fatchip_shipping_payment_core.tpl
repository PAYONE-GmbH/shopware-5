...
<div class="confirm--outer-container">
    <form id="shippingPaymentForm" name="shippingPaymentForm" method="post" action="{url controller='FatchipBSPayoneMasterpassCheckout' action='saveShippingPayment' sTarget='FatchipBSPayoneMasterpassCheckout' sTargetAction='index'}" class="payment">
        ...
        <div class="shipping-payment--information">
            ...
            <div class="confirm--inner-container block">
                {block name='frontend_checkout_shipping_payment_core_payment_fields'}
                    <input type="hidden" name="payment" value="{$fatchipBSPayoneMasterpassID}" />
                {/block}
            </div>
