...
<div class="payment--form-group">
    {if ($moptCreditCardCheckEnvironment.birthdayunderage === "1" || $fcPayolutionConfig.payolutionB2bmode == "0" && $moptCreditCardCheckEnvironment.birthday == "0000-00-00") || ( $fcPayolutionConfig.payolutionB2bmode == 1 && $moptCreditCardCheckEnvironment.birthday == "0000-00-00" && !$sUserData.billingaddress.company  ) }
        ...
        <select name="moptPaymentData[mopt_payone__payolution_installment_birthday]" 
                ...
                class="is--required{if $error_flags.mopt_payone__payolution_installment_birthday || $moptCreditCardCheckEnvironment.birthdayunderage === "1"} has--error{/if}">
            <option value="">--</option>
        ...
        <select name="moptPaymentData[mopt_payone__payolution_installment_birthmonth]" 
                ...
                {if $payment_mean.id == $form_data.payment}required="required" aria-required="true"{/if}
                class="is--required {if $error_flags.mopt_payone__payolution_installment_birthmonth || $moptCreditCardCheckEnvironment.birthdayunderage === "1"} has--error{/if}">
            <option value="">--</option>
        ...
        <select name="moptPaymentData[mopt_payone__payolution_installment_birthyear]" 
                ...
                {if $payment_mean.id == $form_data.payment}required="required" aria-required="true"{/if}
                class="select--country is--required{if $error_flags.mopt_payone__payolution_installment_birthyear || $moptCreditCardCheckEnvironment.birthdayunderage === "1"} register--error-msg {/if}">
            <option value="">----</option>
    ...
    <div id="installment-hint-18-years" class="{if $moptCreditCardCheckEnvironment.birthdayunderage !== "1"}is--hidden{/if} register--error-msg">{s namespace='frontend/MoptPaymentPayone/errorMessages' name="birthdayUnderageError"} Sie müssen mindestens 18 Jahre alt sein, um diese Zahlart verwenden zu können. {/s}</div>        
