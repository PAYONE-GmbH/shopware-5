...
<div class="payment--form-group">
    ...
    <p class="none clearfix">
        <input name="moptPaymentData[mopt_payone__klarna_agreement]" type="checkbox" id="mopt_payone__klarna_agreement" value="true"
               {if $form_data.mopt_payone__klarna_agreement eq "on"}
                   checked="checked"
               {/if}
               class="checkbox{if $error_flags.mopt_payone__klarna_agreement} has--error{/if}"/>
        <label for="mopt_payone__klarna_agreement"  style="float:none; width:100%; display:inline">{$moptCreditCardCheckEnvironment.moptKlarnaInformation.consent}</label>
    </p>
