...
<div class="payment--form-group">
    ...
    {if ($fcPayolutionConfigDebitnote.payolutionB2bmode == "0" && $moptCreditCardCheckEnvironment.birthday == "0000-00-00") || ( $fcPayolutionConfigDebitnote.payolutionB2bmode == 1 && $moptCreditCardCheckEnvironment.birthday == "0000-00-00" && !$sUserData.billingaddress.company  ) }
        <p class ="none">
            <label for="mopt_payone__fin_payolution_debitnote_birthday">
                {s name='birthdate'}Geburtsdatum{/s}
            </label>
        </p>

        <div class="select-field">
            <select name="moptPaymentData[mopt_payone__fin_payolution_debitnote_birthday]"
                    id="mopt_payone__fin_payolution_debitnote_birthday" onchange="payolutionDebitNoteDobInput()"
                    aria-label="{s name='birthdate'}Geburtsdatum{/s}"
                    {if $payment_mean.id == $form_data.payment}required="required" aria-required="true"{/if}
                    class="payment--field {if $error_flags.mopt_payone__fin_payolution_debitnote_birthday} has--error{/if}">
                <option value="">--</option>
                {section name="birthdate" start=1 loop=32 step=1}
                    <option value="{if $smarty.section.birthdate.index < 10}0{/if}{$smarty.section.birthdate.index}"
                            {if $smarty.section.birthdate.index eq $moptCreditCardCheckEnvironment.mopt_payone__fin_payolution_debitnote_birthday}
                                selected
                            {/if}>
    ...
    <div class="select-field">
            <select name="moptPaymentData[mopt_payone__fin_payolution_debitnote_birthmonth]"
                    id="mopt_payone__fin_payolution_debitnote_birthmonth" onchange="payolutionDebitNoteDobInput()"
                    aria-label="{s name='birthdate'}Geburtsdatum{/s}"
                    {if $payment_mean.id == $form_data.payment}required="required" aria-required="true"{/if}
                    class="payment--field {if $error_flags.mopt_payone__fin_payolution_debitnote_birthmonth} has--error{/if}">
                <option value="">--</option>
                {section name="birthmonth" start=1 loop=13 step=1}
                    <option value="{if $smarty.section.birthmonth.index < 10}0{/if}{$smarty.section.birthmonth.index}"
                            {if $smarty.section.birthmonth.index eq $moptCreditCardCheckEnvironment.mopt_payone__fin_payolution_debitnote_birthmonth}
                                selected
                            {/if}>
    ...
    <div class="select-field">
        <select name="moptPaymentData[mopt_payone__fin_payolution_debitnote_birthyear]"
                id="mopt_payone__fin_payolution_debitnote_birthyear" onchange="payolutionDebitNoteDobInput()"
                aria-label="{s name='birthdate'}Geburtsdatum{/s}"
                {if $payment_mean.id == $form_data.payment}required="required" aria-required="true"{/if}
                class="payment--field {if $error_flags.mopt_payone__fin_payolution_debitnote_birthyear} has--error{/if}">
            <option value="">----</option>
            {section name="birthyear" loop=2016 max=100 step=-1}
                <option value="{$smarty.section.birthyear.index}" 
                        {if $smarty.section.birthyear.index eq $moptCreditCardCheckEnvironment.mopt_payone__fin_payolution_debitnote_birthyear}
                            selected
                        {/if}>
    ...
    <input class="is--hidden validate-18-years" type="text" name="moptPaymentData[mopt_payone__fin_payolution_debitnote_birthdaydate]" id="mopt_payone__fin_payolution_debitnote_birthdaydate" value="{$moptCreditCardCheckEnvironment.birthday}">
    ...
    {if $fcPayolutionConfigDebitnote.payolutionB2bmode && $sUserData.billingaddress.company}
        ...
        <input class="is--hidden" type="text" name="moptPaymentData[mopt_payone__fin_payolution_debitnote_b2bmode]" id="moptPaymentData[mopt_payone__fin_payolution_debitnote_b2bmode]" value="1">
    {/if}    

    <input name="moptPaymentData[mopt_payone__fin_payolution_debitnote_iban]"
           type="text"
           id="mopt_payone__fin_payolution_debitnote_iban"
           ...
           value="{$form_data.mopt_payone__fin_payolution_debitnote_iban|escape}"
           ...
           class="payment--field {if $moptRequired}is--required{/if}{if $error_flags.mopt_payone__fin_payolution_debitnote_iban} has--error{/if} moptPayoneIban" />

    <p class="none clearfix">
        <input name="moptPaymentData[mopt_payone__fin_payolution_debitnote_agreement]" type="checkbox" id="mopt_payone__fin_payolution_debitnote_agreement" value="true"
               class="checkbox"/>
        <label class="{if $error_flags.mopt_payone__fin_payolution_debitnote_agreement} has--error {/if}" for="mopt_payone__fin_payolution_debitnote_agreement"  style="float:none; width:100%; display:inline">{$moptCreditCardCheckEnvironment.moptPayolutionInformation.consentDebit}</label>
    </p>
    ...
    <p class="none clearfix">
        <input name="moptPaymentData[mopt_payone__fin_payolution_debitnote_agreement2]" type="checkbox" id="mopt_payone__fin_payolution_debitnote_agreement2" value="true"
               {if $form_data.mopt_payone__fin_payolution_debitnote_agreement2 eq "on"}
                   checked="checked"
               {/if}
               class="checkbox"/>
        <label class="{if $error_flags.mopt_payone__fin_payolution_debitnote_agreement2} has--error{/if}" for="mopt_payone__fin_payolution_debitnote_agreement2"  style="float:none; width:100%; display:inline">{$moptCreditCardCheckEnvironment.moptPayolutionInformation.sepaagreement}</label>
    </p>  
...
<script type="text/javascript">
    ...
    {
        var daySelect = document.getElementById("mopt_payone__fin_payolution_debitnote_birthday");
        var monthSelect = document.getElementById("mopt_payone__fin_payolution_debitnote_birthmonth");
        var yearSelect = document.getElementById('mopt_payone__fin_payolution_debitnote_birthyear');
        var hiddenDobFull = document.getElementById("mopt_payone__fin_payolution_debitnote_birthdaydate");
