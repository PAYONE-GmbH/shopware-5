...
function moptPaymentReady() {
    ...
    $.plugin('moptPayoneIbanBicValidator', {
        defaults: {
            ...
            moptIbanWrongCharacterMessage: 'Dieses Feld darf nur Großbuchstaben und Ziffern enthalten',
            moptIbanWrongLengthMessage: 'Bitte prüfen Sie die Länge der IBAN',
            moptIbanWrongCecksumMessage: 'Die Prüfsumme der IBAN ist falsch'
        },
        init: function () {
            var me = this;
            ...
            me.$el.bind('keyup change', function (e) {
                ...
                var check = me.isValidIBANNumber(me.$el.val());
                if (me.$el.val() && !me.opts.ibanbicReg.test(me.$el.val())) {
                    me.$el.addClass('has--error');
                    $('<div>', {
                        'html': '<p>' + me.opts.moptIbanWrongCharacterMessage + '</p>',
                        'id': 'moptiban--message',
                        'class': me.opts.errorMessageClass
                    }).insertAfter(me.$el);
                } else if (me.$el.val() && check === false) {
                    me.$el.addClass('has--error');
                    $('<div>', {
                        'html': '<p>' + me.opts.moptIbanWrongLengthMessage + '</p>',
                        'id': 'moptiban--message',
                        'class': me.opts.errorMessageClass
                    }).insertAfter(me.$el);
                } else if (me.$el.val() && check !== 1) {
                    me.$el.addClass('has--error');
                    $('<div>', {
                        'html': '<p>' + me.opts.moptIbanWrongCecksumMessage + '</p>',
                        'id': 'moptiban--message',
                        'class': me.opts.errorMessageClass
                    }).insertAfter(me.$el);
                } else {
